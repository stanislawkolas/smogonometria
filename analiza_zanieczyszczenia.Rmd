...
1.  Jakie czynniki społeczno-ekonomiczne i przestrzenne wpływają na poziom zanieczyszczenia powietrza w polskich powiatach?

2.  Czy zanieczyszczenie powietrza wykazuje przestrzenną autokorelację, tworząc klastry wysokich i niskich wartości?

3.  Jakie modele ekonometryczne najlepiej opisują przestrzenne zależności w zanieczyszczeniu powietrza?

## Uzasadnienie wyboru:

-   Zanieczyszczenie powietrza to główny problem zdrowia publicznego w Polsce
-   Konieczność koordynacji polityk regionalnych
-   Przestrzenny charakter zjawiska wymaga specjalistycznych metod analizy
...
# Sprawdzenie poprawności wczytania pliku .shp
...
# Boxplot – Wskaznik (niebieski)
ggplot(Dane, aes(x = "1", y = Wskaznik)) +
  geom_boxplot(fill = "steelblue", outlier.color = "red", outlier.shape = 8) +
  labs(title = "Boxplot - Wskaznik", y = "Wskaznik", x = "") +
  theme_minimal()

# Boxplot – Grunty leśne (zielonkawy)
ggplot(Dane, aes(x = "1", y = Grunty_lesne)) +
  geom_boxplot(fill = "olivedrab", outlier.color = "red", outlier.shape = 8) +
  labs(title = "Boxplot - Grunty leśne", y = "Grunty leśne", x = "") +
  theme_minimal()

# Boxplot – Powierzchnia powiatu (fioletowy)
ggplot(Dane, aes(x = "1", y = Pow_powiatu)) +
  geom_boxplot(fill = "mediumpurple", outlier.color = "red", outlier.shape = 8) +
  labs(title = "Boxplot - Powierzchnia powiatu", y = "Powierzchnia powiatu", x = "") +
  theme_minimal()
...
## Histogram wskaźnika zanieczyszczenia powietrza
...
## Kartogramy wybranych zmiennych objaśniających
...
Model OLS wykazuje umiarkowane dopasowanie (R² ≈ 0.52), jednak test Morana I dla reszt (I = 0.34, *p* \< 2.2e‑16) wskazuje na istotną przestrzenną autokorelację, co sugeruje, że klasyczna regresja nie uwzględnia zależności przestrzennych między powiatami. Współczynniki regresji są zgodne z intuicją: większa powierzchnia powiatu i tereny zieleni obniżają zanieczyszczenie, podczas gdy gęstość zaludnienia i liczba pojazdów je zwiększają. Jednakże, brak istotności statystycznej dla wielu zmiennych sugeruje potrzebę zastosowania bardziej zaawansowanego modelowania przestrzennego.
...
Wsparcie polityki regionalnej: Lokalne oszacowania parametrów dostarczą bardziej precyzyjnych informacji dla decydentów na poziomie powiatowym i wojewódzkim, umożliwiając dostosowanie strategii redukcji zanieczyszczeń do specyfiki danego regionu. Jak wykazali Lu et al. (2014), dopasowanie modelu GWR (mierzone R²) jest znacznie lepsze niż klasycznej regresji OLS dla analiz zanieczyszczeń powietrza. Komplementarność z modelami globalnymi: GWR nie zastępuje, lecz uzupełnia globalne modele przestrzenne, dostarczając informacji o lokalnych odchyleniach od średnich efektów oszacowanych w modelach SAR czy SDM (Wang et al., 2019).
...
Fotheringham, A. S., Brunsdon, C., & Charlton, M. (2002). Geographically weighted regression: The analysis of spatially varying relationships. John Wiley & Sons. Holnicki, P., Tainio, M., Kałuszko, A., & Nahorski, Z. (2018). Burden of disease due to air pollution in Warsaw, Poland. International Journal of Environmental Research and Public Health, 15(9), 1-16.
